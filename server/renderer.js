...
import routes                         from '../app/routing/serverRoutes.js';
  ...
  const need = currentRoute.map(({ route, match }) => {
    if (route.component) {
      return route.component.loadData ?
        route.component.loadData( store, match, location, qs.parse(match.params['0'], { ignoreQueryPrefix: true }) ) : Promise.resolve(null);
    }
